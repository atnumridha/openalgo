...
def handle_auth_success(auth_token, user_session_key, broker, feed_token=None, user_id=None):
    ...
    """
    # A successful broker authentication can replace credentials or even the
    # selected identity. Live automation must be approved again afterwards.
    from services.strategy_module import live_authorization

    live_authorization.invalidate_for_reauthentication(user_session_key)
    previous_username = session.get("user")
    if previous_username and previous_username != user_session_key:
        live_authorization.invalidate_for_reauthentication(previous_username)
