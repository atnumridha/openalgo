"""Audit and broadcast strategy events; route routine WhatsApp alerts.

Critical safety events enter the durable outbox inside the database audit
transaction. Routine notifications remain best-effort and WhatsApp remains
downstream of an order or protective exit.
"""
...
def record_and_notify(
    ...
    **fields: Any,
    ...
    """Record, stream, then asynchronously notify routine allowlisted events.
    ...
    if kind in AUTOMATION_EVENT_KINDS:
        logger.warning("Refusing account event kind %r on the strategy event stream", kind)
        return None
    try:
    ...
    try:
        ...
        logger.exception(
            "Could not serialise lifecycle event %s for strategy %s", kind, strategy_id
        )
    ...
    try:
        ...
        logger.exception(
            "Could not broadcast lifecycle event %s for strategy %s", kind, strategy_id
        )

    # Critical events have already been enqueued transactionally with the
    # audit row. A second immediate send would duplicate the safety alert.
    if kind in store.CRITICAL_ALERT_EVENT_KINDS or kind not in STRATEGY_MATERIAL_EVENT_KINDS:
        return row
    ...
    try:
        ...
        logger.exception(
            "Could not queue WhatsApp lifecycle event %s for strategy %s", kind, strategy_id
        )
    return row
...
def record_user_and_notify(
    ...
    **fields: Any,
    ...
    if kind not in AUTOMATION_EVENT_KINDS:
        ...
        return None
    try:
        audit_fields = {key: value for key, value in fields.items() if key in _USER_AUDIT_FIELDS}
    ...
    if kind in store.CRITICAL_ALERT_AUTOMATION_KINDS or kind not in AUTOMATION_MATERIAL_EVENT_KINDS:
        return row
