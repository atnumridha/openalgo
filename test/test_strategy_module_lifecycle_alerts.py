...
def test_material_lifecycle_events_enqueue_a_whatsapp_alert(kind):
    ...
    if kind in lifecycle_events.store.CRITICAL_ALERT_EVENT_KINDS:
        # The store commits a durable outbox row with the audit event. The
        # lifecycle wrapper must not send a duplicate best-effort alert.
...
def test_a_user_scoped_lifecycle_event_needs_no_fabricated_strategy_id(kind):
    ...
    if kind in lifecycle_events.store.CRITICAL_ALERT_AUTOMATION_KINDS:
        notifier.send_strategy_lifecycle_alert.assert_not_called()
    else:
