...
from services.strategy_module.session import IST, session_reset_time, session_started_at
...
def authorization_expires_at(session_day: str) -> str:
    """The instant this session's grant stops being valid, in IST.

    A session day runs from the platform reset until the next reset. The
    default reset is 03:00, so that boundary is the following calendar day.
    A reset at or after noon is the same calendar day as the session date:
    adding a day there would tell the operator the grant lasts until tomorrow
    when enforcement already ended it today.
    """
    anchor = IST.localize(datetime.fromisoformat(f"{session_day}T12:00:00"))
    boundary = session_started_at(anchor) + timedelta(days=1)
    reset = session_reset_time()
